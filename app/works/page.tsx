...
export default function WorksPage() {
  ...
  useMotionValueEvent(scrollYProgress, "change", (latest) => {
    ...
    // We have total projects + a 50vh spacer at the end.
    // Let's divide the space by number of projects.
  ...
  return (
    <div 
      className="bg-background text-foreground min-h-screen overflow-x-hidden scroll-smooth bg-[radial-gradient(ellipse_at_top_right,_var(--primary-rgb),_transparent_40%)]"
    >
      ...
      <main ref={containerRef} className="relative w-full">
        ...
        <section className="h-[50vh] w-full flex flex-col items-center justify-center border-t border-border/10 bg-background/50 backdrop-blur-3xl ml-24 md:ml-32">
             <div className="text-center space-y-6">
                <p className="font-mono text-[10px] tracking-[0.6em] text-muted-foreground uppercase opacity-50">END OF SHOWCASE</p>
                <h2 className="text-4xl md:text-6xl font-light tracking-tighter uppercase opacity-20">NEXT: CONTACT</h2>
...
function ProjectSection({ project, index, isLast }: { project: any, index: number, isLast: boolean }) {
  const sectionRef = useRef<HTMLElement>(null);
  
  // Transform logic for fade-out as it scrolls away
  ...
  // stays at full opacity until it is 70% scrolled out
  const opacity = useTransform(scrollYProgress, [0, 0.7, 1], isLast ? [1, 1, 1] : [1, 1, 0]);
  const scale = useTransform(scrollYProgress, [0, 1], isLast ? [1, 1] : [1, 0.95]);
  const y = useTransform(scrollYProgress, [0, 1], isLast ? [0, 0] : [0, -150]);
  ...
  return (
    <motion.section 
      ...
      className="project-section relative min-h-[100dvh] lg:h-screen w-full flex items-center justify-center px-4 py-32 lg:py-0 md:px-24 overflow-hidden bg-background"
    >
      {/* Background Index */}
      <div className="absolute inset-0 flex items-center justify-end pointer-events-none select-none overflow-hidden">
        <motion.span
          initial={{ opacity: 0, x: 100, scale: 0.8 }}
          whileInView={{ opacity: 0.04, x: 0, scale: 1 }}
          viewport={{ once: true }}
          transition={{ duration: 1.5, ease: [0.16, 1, 0.3, 1] }}
          className="text-[60vw] md:text-[45vw] font-black italic uppercase leading-none tracking-tighter pr-2 md:pr-16"
        >
          {index + 1}
        </motion.span>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.15fr] gap-12 lg:gap-20 items-center w-full max-w-7xl relative z-10 pl-0 md:pl-24">
        {/* Project metadata */}
        <div className="space-y-10">
          ...
          >
            {project.tags.map((tag: string) => (
              <span key={tag} className="px-3 py-1 border border-border/50 rounded-full font-mono text-[9px] tracking-[0.3em] uppercase text-muted-foreground">
                {tag}
              </span>
            ))}
          </motion.div>

          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.45 }}>
            <a href={project.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 group font-mono text-[10px] tracking-[0.4em] uppercase hover:text-primary transition-colors duration-300">
        ...
        >
          <div className="relative aspect-[4/3] md:aspect-[16/10] overflow-hidden bg-foreground/5 border border-border shadow-2xl">
            <motion.img
              ...
              className="w-full h-full object-cover transition-all duration-700 group-hover:scale-[1.25] grayscale-[0.5] group-hover:grayscale-0 contrast-[1.1]"
            />
